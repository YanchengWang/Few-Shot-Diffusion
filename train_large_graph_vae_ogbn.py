...
def train(params:argparse.Namespace):
    ...
    for epc in range(lastepc, params.epoch):
        ...
        if (epc + 1) % params.intervalplot == 0:
            ...
            utils.plot_tsne(all_gt, gen_labels, gen_embs, gen_labels, params.samdir, epc, f"map_{params.coef_map}")
...
def decode(params:argparse.Namespace):
    ...
    np.save(os.path.join(params.samdir, f'{params.dataset}_latents_{allneighbor_maps.shape[0]}_lgvae_{params.lastepo}_{data_decode.shape[-1]}_decode_map.npy'),allneighbor_maps)
...
def main():
    ...
    parser.add_argument('--neighbor_map_dim',type=int,default=101, help='max num of nodes in an assigned cluster')    
    parser.add_argument('--clusterdir',type=str,default='/home/local/ASUAD/changyu2/generate_graph_embbedding/gcl_embeddings/cora/all_data/all_ps_cluster_kmeans.npy',help='assigned_cluster dir')
    parser.add_argument('--cluster_num',type=int,default=27,help='number of assigned clusters')
