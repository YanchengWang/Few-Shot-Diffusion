...
genum = (datalen//genbatch + 1 ) *  genbatch * 2
...
def command(args):
    ...
    script_vae_encode = f'CUDA_VISIBLE_DEVICES={args.gpu_id} {python_path}  {root_dir}/train_large_graph_vae.py --moddir {vae_dir_3} --samdir {vae_dir_3} --datadir {embd_dir} --labeldir {label_dir} --adjdir {adj_dir} --clusterdir {clusterdir} --dataset {args.dataset} --neighbor_map_dim {neighbor_map_dim} --batchsize {batchsize} --run_type encode --lastepo {epoch_3_vae} --feat_emb_dim {feat_emb_dim} --cluster_num {cluster_num}'
    ...
    script_vae_decode = f'CUDA_VISIBLE_DEVICES={args.gpu_id} {python_path}  {root_dir}/train_large_graph_vae.py --moddir {vae_dir_3} --samdir {vae_dir_3} --datadir {embd_dir} --labeldir {label_dir}   --adjdir {adj_dir} --clusterdir {clusterdir} --dataset {args.dataset} --neighbor_map_dim {neighbor_map_dim} --batchsize {batchsize} --run_type decode --lastepo {epoch_3_vae} --datadecode_dir {diffusion_sample_data} --labelsdecode_dir {diffusion_sample_label} --feat_emb_dim {feat_emb_dim} --cluster_num {cluster_num}'
