...
class AttnBlock(nn.Module):
    def __init__(self, in_ch:int):
        ...
        self.proj_q = nn.Conv1d(in_ch, in_ch, kernel_size = 1, stride=1, padding=0)
        self.proj_k = nn.Conv1d(in_ch, in_ch, kernel_size = 1, stride=1, padding=0)
        self.proj_v = nn.Conv1d(in_ch, in_ch, kernel_size = 1, stride=1, padding=0)
        self.proj = nn.Conv1d(in_ch, in_ch, kernel_size = 1, stride=1, padding=0)
    ...
    def forward(self, x:torch.Tensor) -> torch.Tensor:
        ...
        assert list(w.shape) == [B, W, W]
        w = F.softmax(w, dim=-1)

        v = v.permute(0, 2, 1)
        h = torch.bmm(w, v)
        assert list(h.shape) == [B, W, C]
        h = h.permute(0, 2, 1)
        h = self.proj(h)
...
class Unet_1d(nn.Module):
    def __init__(self, in_ch=1, mod_ch=64, out_ch=1, ch_mul=[1,2,4,8], num_res_blocks=2, cdim=10, use_conv=True, droprate=0, dtype=torch.float32):
        ...
        self.downblocks = nn.ModuleList([
            EmbedSequential(nn.Conv1d(in_ch, self.mod_ch, 3, padding=1))
        ])
        ...
        self.out = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv1d(now_ch, self.out_ch, 3, stride = 1, padding = 1)
        )
